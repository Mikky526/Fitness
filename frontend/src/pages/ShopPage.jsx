...
const TYPE_LABELS = {
  membership:   { label: 'Membership',   color: 'bg-indigo-100 text-indigo-700' },
  ...
  return (
    <TiltCard>
      <motion.div
        ...
        className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden hover:border-indigo-100/60 flex flex-col h-full"
      >
        ...
        <div className="p-6 flex-1 flex flex-col">
          <span className="bg-gray-100 text-gray-600 text-xs font-bold px-2.5 py-1 rounded-full w-fit">
            {typeInfo.label}
          </span>

          <h3 className="text-xl font-bold text-gray-900 mt-2 leading-snug">{product.name}</h3>
          <p className="text-gray-500 text-sm leading-relaxed mt-2 flex-1">{product.description}</p>
          ...
          <AnimatePresence mode="wait">
            {added ? (
              <motion.div
                ...
                className="w-full bg-emerald-500 text-white font-bold py-3 rounded-2xl shadow-md flex items-center justify-center gap-2 mt-4"
              >
                <span>✓</span> Added!
              </motion.div>
            ) : (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={handleAdd}
                className={`w-full bg-gradient-to-r ${product.gradient || product.color} text-white font-bold py-3 rounded-2xl shadow-md hover:shadow-lg transition-all mt-4`}
              >
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="bg-gradient-to-br from-slate-900 via-indigo-950 to-purple-900 rounded-3xl p-10 mx-4 mt-4 relative overflow-hidden"
      >
        {/* Floating particles */}
        ...
        <motion.div
          ...
          className="absolute top-0 right-0 w-72 h-72 bg-indigo-400 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"
        />
        <motion.div
          animate={{ scale: [1, 1.5, 0.8, 1.3, 1], x: [0, -40, 20, -15, 0], opacity: [0.08, 0.2, 0.05, 0.18, 0.08] }}
          transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut', delay: 1.5 }}
          className="absolute bottom-0 left-0 w-64 h-64 bg-purple-400 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2"
        />
        <motion.div
          animate={{ scale: [1, 1.5, 0.8, 1.3, 1], opacity: [0.08, 0.2, 0.05, 0.18, 0.08] }}
          transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut', delay: 3 }}
          className="absolute top-1/2 left-1/2 w-48 h-48 bg-pink-400 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2"
        />

        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            {/* Top pill */}
            <div className="inline-flex items-center gap-2 bg-white/10 border border-white/20 text-indigo-200 text-xs font-bold px-3 py-1.5 rounded-full mb-4">
              <span>★</span>
            ...
            <p className="text-indigo-200/80 text-sm mt-2">Choose a plan that fits your goals. All purchases are instant.</p>
          </div>
            ...
            >
              ...
              {count > 0 && (
                <motion.span
                  ...
                  initial={{ scale: 0.5 }}
                  animate={{ scale: 1 }}
                  className="w-6 h-6 rounded-full bg-gradient-to-r from-indigo-400 to-purple-400 text-white text-xs font-extrabold flex items-center justify-center"
                >
                  {count}
                </motion.span>
              )}
      ...
      <motion.div
        ...
        className="bg-white rounded-2xl p-2 shadow-sm border border-gray-100 flex gap-2 flex-wrap mx-4 my-6"
      >
        {FILTERS.map(f => (
          <motion.button
            key={f}
